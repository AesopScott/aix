...
- Actual guest/member registration rows are treated as registered only when their lifecycle status is empty or explicitly registered/attended. Explicit workflow statuses such as `new`, `pending-engagement`, `confirmed`, `invited`, and `active` are not public registration evidence.
