...
Browser uploads use the R2 multipart API through `/api/storage` instead of sending the selected file through a single `multipart/form-data` request. The `/files/` page sends only metadata in the create request, uploads non-empty files as 16 MiB raw parts, then completes or aborts the multipart session from the same authenticated API. Zero-byte files can still use the simple upload path. This avoids Cloudflare request body and form parser failures for PDFs, office documents, video drafts, and other media while preserving the same object naming and metadata rules.
