...
Registration records include name, company, title, industry, company email, phone, phone verification status, and separate publication-use opt-ins for name and company. Successful member, guest, and partner registrations upsert one canonical contact record under `crm:contact:{email}` with `id` set to the normalized email address, so duplicate email addresses cannot create duplicate contacts. Each registration appends or updates an entry in the contact's `events` history with event metadata, registration type/role, invite code, registration ID, and manual attendance fields. Company rollups are stored under `crm:company:{company-slug}` and `{type}-company:{company-slug}` so contact views have email, phone, company, and title immediately.
