...
Registration records include name, company, title, industry, company email, phone, phone verification status, and separate publication-use opt-ins for name and company. Successful member, guest, and partner registrations also upsert contact records under `crm:contact:{email}` and `{type}-contact:{email}`, plus company records under `crm:company:{company-slug}` and `{type}-company:{company-slug}` so contact views have email, phone, company, and title immediately.
