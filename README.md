...
The member registration page lives at `/member-registration/` and hides the registration form behind a six-digit member invite code. Codes can be passed as `/member-registration/?invite=######`, stored in browser session storage by the home page invite-code modal, or entered directly on the page gate. For administrative page review only, `/member-registration/?preview=4321` unlocks the form in non-submitting preview mode and blocks SMS verification calls.
Member registrations are stored in Cloudflare R2 under `crm/registrations/member/` and can be mirrored into the `MOJO_SUMMITS_SETUP_STATE` KV namespace under `crm:member-registrant:*` when `MOJO_REGISTRATION_KV_MIRROR=true`.
...
Registration records include name, company, title, industry, email, LinkedIn profile URL, optional brief photo, phone, phone verification status, and separate publication-use opt-ins for name and company. Brief photos are required for featured guest-style roles such as featured guests, featured authors, featured members, featured partners, presenters, and round table leaders. Gmail and Googlemail addresses are allowed for registration. Phone is required, but the registration handler does not verify phone accuracy. Successful member, guest, and partner registrations post through `/api/{type}-registration`, store CRM rows in Cloudflare R2 under `crm/registrations/{type}/`, store submitted brief photo objects under `crm/registration-photos/{type}/`, and can mirror copies into `MOJO_SUMMITS_SETUP_STATE` under `crm:{type}-registrant:{createdAt}:{uuid}` when `MOJO_REGISTRATION_KV_MIRROR=true`. Registration handling also upserts one canonical contact record under `crm:contact:{email}` with `id` set to the normalized email address, so duplicate email addresses cannot create duplicate contacts. Each registration appends or updates an entry in the contact's `events` history with event metadata, registration type/role, invite code, registration ID, and manual attendance fields. The CRM Contacts view reads `crm:contact:*` and sequentially rebuilds contacts from member, guest, and partner registrants before listing them so contact history cannot lag behind registration rows. Company rollups are stored under `crm:company:{company-slug}` and `{type}-company:{company-slug}` so contact views have email, phone, company, title, LinkedIn profile URL, and brief photo metadata immediately when available.
...
The public partners page lives at `/partners/` and positions Mojo AI Summits as an executive relationship and intelligence network rather than an event sponsorship seller. It explains Strategic Intelligence Partner value, Partner Candidate entry, Research Partner, Summit Partner, Strategic Partner, trust rules, and participation model.
...
The Blinq/Zapier CRM capture endpoint lives at `/api/blinq-contact`. It accepts authenticated Zapier POST payloads with `name`, `job_title`, `company`, `email`, and `profile_photo`, plus optional `phone`, then upserts the appropriate Guest or Partner Matrix prospect row and links the canonical `crm:contact:{email}` record back to that matrix row. Configure `MOJO_BLINQ_ZAPIER_API_KEY` as the preferred Cloudflare secret and send it from Zapier as `?token=<api-key>`, `Authorization: Bearer <api-key>`, or `x-api-key`. See `docs/processes/blinq-zapier-crm-endpoint.md`.
