...
- Registration data model: member and guest registration collect name, company, title, industry, company email, phone, phone verification status, and separate publication-use opt-ins for name and company. Title and industry do not have publication opt-ins. Successful member, guest, and partner registrations upsert one canonical `crm:contact:{email}` record with `id` set to the normalized email address, then append/update that contact's `events` history with the event metadata, registration type/role, invite code, registration ID, and manual attendance fields. Company rollups still use `crm:company:{company-slug}` / `{type}-company:{company-slug}` records so contact views have email, phone, company, and title immediately.
