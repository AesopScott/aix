...
- Registration data model: member and guest registration collect name, company, title, industry, company email, phone, phone verification status, and separate publication-use opt-ins for name and company. Title and industry do not have publication opt-ins. Successful member, guest, and partner registrations upsert `crm:contact:{email}` / `{type}-contact:{email}` and `crm:company:{company-slug}` / `{type}-company:{company-slug}` records so contact views have email, phone, company, and title immediately.
...
- Storage model: CRM creates single-use codes under `crm:partner-invite-code:######`. Successful registrations are stored under `partner-registration:*` and `crm:partner-registrant:*`, with event metadata copied from the invite code. Partner registration also upserts `partner-contact:{email}` and `partner-company:{company-slug}` for partner profile access compatibility.
