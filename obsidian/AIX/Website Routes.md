...
- Registration data model: member and guest registration collect name, company, title, industry, company email, phone, phone verification status, and separate publication-use opt-ins for name and company. Title and industry do not have publication opt-ins. Phone is required, but the registration handler does not verify phone accuracy. Successful member, guest, and partner registrations post through `/api/{type}-registration`, store CRM rows in `MOJO_SUMMITS_SETUP_STATE` under `crm:{type}-registrant:{createdAt}:{uuid}` with legacy copies under `{type}-registration:{createdAt}:{uuid}`, upsert one canonical `crm:contact:{email}` record with `id` set to the normalized email address, then append/update that contact's `events` history with the event metadata, virtual show slot metadata when present, registration type/role, invite code, registration ID, and manual attendance fields. The CRM Contacts view reads `crm:contact:*` and sequentially rebuilds contacts from member, guest, and partner registrants before listing them so contact history cannot lag behind registration rows. Company rollups still use `crm:company:{company-slug}` / `{type}-company:{company-slug}` records so contact views have email, phone, company, and title immediately.
- Event-tied invite codes: `/crm/` reads upcoming generated event pages from `event-playbooks:index`. Staff select an event when generating guest, member, or partner invite links. Guest/member invite links also carry a show selection: both shows, morning show, or afternoon show. Guest/member codes are stored under `crm:guest-invite-code:######` and `crm:member-invite-code:######`; partner codes remain under `crm:partner-invite-code:######`. Registration submissions copy `eventSlug`, `eventName`, `eventDate`, and show metadata from the invite code into the CRM registrant record, and `/events/[slug]/` shows an event registrant roster for matching member, guest, and partner records.
...
- Storage model: CRM creates single-use codes under `crm:partner-invite-code:######`. Successful registrations are stored under `partner-registration:*` and `crm:partner-registrant:*`, with event metadata copied from the invite code. Partner registration upserts the canonical `crm:contact:{email}` record and `partner-company:{company-slug}` for partner profile access compatibility.
