...
The CRM is the source of truth for public featured lineups. Each event time slot can publish at most 6 Featured Guests, 1 Featured Author, and 2 Featured Sponsors/Partners. The website can publish a featured person only when the person comes from a submitted registration row or a registered/attended CRM contact-event row for that event/show. Public aggregate featured lists are flattened from those show-level lineups so unassigned-show featured records do not leak onto the website. Invite links, non-registered contact event history, and matrix-only prospects can support tracking, but they do not publish someone to the website lineup by themselves. If an invite/contact record disagrees with the current CRM registration/matrix state, the CRM registration/matrix state wins.
